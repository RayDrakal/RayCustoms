...
function s.initial_effect(c)
	...
	e2:SetValue(CARD_SUPERIOR_ARGOS)
	c:RegisterEffect(e2)
    --Remove 1 card from your Deck / Grave
	local e3=Effect.CreateEffect(c)
	e3:SetDescription(aux.Stringid(id,1))
	e3:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e3:SetProperty(EFFECT_FLAG_DELAY)
	e3:SetCode(EVENT_SUMMON_SUCCESS|EVENT_SPSUMMON_SUCCESS)
...
function s.rmop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,s.rmfilter,tp,LLOCATION_DECK+LOCATION_GRAVE,0,1,1,nil)
	if #g>0 then
