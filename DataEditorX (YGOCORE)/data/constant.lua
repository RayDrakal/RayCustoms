...
--Locations 区域
LOCATION_DECK		=0x01		--덱
LOCATION_HAND		=0x02		--핸드
LOCATION_MZONE		=0x04		--몬스터존
LOCATION_SZONE		=0x08		--마법 / 함정
LOCATION_GRAVE		=0x10		--묘지
LOCATION_REMOVED	=0x20		--제외
LOCATION_EXTRA		=0x40		--엑스트라덱
LOCATION_OVERLAY	=0x80		--엑시즈 소재
LOCATION_ONFIELD	=0x0c		--필드 위（LOCATION_MZONE+LOCATION_SZONE）
--Locations (for redirect) 若在重定向类效果中指定LOCATION_DECK则为弹回卡组顶部
LOCATION_DECKBOT	=0x10001	--덱 아래
LOCATION_DECKSHF	=0x20001	--덱 위
--Locations of spell cards
LOCATION_FZONE		=0x100		--필드 존
LOCATION_PZONE		=0x200		--펜듈럼 존
--Positions 表示形式
POS_FACEUP_ATTACK		=0x1	--앞면 공격 표시
POS_FACEDOWN_ATTACK		=0x2	--(reserved)
POS_FACEUP_DEFENSE		=0x4	--앞면 수비 표시
POS_FACEDOWN_DEFENSE	=0x8	--뒷면 수비 표시
POS_FACEUP				=0x5	--앞면 표시
POS_FACEDOWN			=0xa	--뒷면 표시
POS_ATTACK				=0x3	--공격 표시
POS_DEFENSE				=0xc	--수비 표시
NO_FLIP_EFFECT			=0x10000--리버스 효과 발동 안함
--Types 卡片类型
TYPE_MONSTER		=0x1		--몬스터
TYPE_SPELL			=0x2		--마법
TYPE_TRAP			=0x4		--함정
TYPE_NORMAL			=0x10		--일반
TYPE_EFFECT			=0x20		--효과
TYPE_FUSION			=0x40		--융합
TYPE_RITUAL			=0x80		--의식
TYPE_TRAPMONSTER	=0x100		--함정 몬스터
TYPE_SPIRIT			=0x200		--스피릿
TYPE_UNION			=0x400		--유니온
TYPE_DUAL			=0x800		--듀얼
TYPE_TUNER			=0x1000		--튜너
TYPE_SYNCHRO		=0x2000		--싱크로
TYPE_TOKEN			=0x4000		--토큰
TYPE_QUICKPLAY		=0x10000	--속공
TYPE_CONTINUOUS		=0x20000	--지속
TYPE_EQUIP			=0x40000	--장착
TYPE_FIELD			=0x80000	--필드
TYPE_COUNTER		=0x100000	--카운터
TYPE_FLIP			=0x200000	--리버스
TYPE_TOON			=0x400000	--툰
TYPE_XYZ			=0x800000	--엑시즈
TYPE_PENDULUM		=0x1000000	--펜듈럼
TYPE_SPSUMMON		=0x2000000	--특수소환
TYPE_LINK			=0x4000000	--링크
--Attributes 属性
ATTRIBUTE_EARTH		=0x01		--땅
ATTRIBUTE_WATER		=0x02		--물
ATTRIBUTE_FIRE		=0x04		--불
ATTRIBUTE_WIND		=0x08		--바람
ATTRIBUTE_LIGHT		=0x10		--빛
ATTRIBUTE_DARK		=0x20		--어둠
ATTRIBUTE_DIVINE	=0x40		--신
--Races 种族
RACE_ALL			=0x1ffffff	--모든 종족
RACE_WARRIOR		=0x1		--전사족
RACE_SPELLCASTER	=0x2		--마법사족
RACE_FAIRY			=0x4		--천사족
RACE_FIEND			=0x8		--악마족
RACE_ZOMBIE			=0x10		--언데드족
RACE_MACHINE		=0x20		--기계족
RACE_AQUA			=0x40		--물족
RACE_PYRO			=0x80		--화염족
RACE_ROCK			=0x100		--암석족
RACE_WINDBEAST		=0x200		--비행야수족
RACE_PLANT			=0x400		--식물족
RACE_INSECT			=0x800		--곤충족
RACE_THUNDER		=0x1000		--번개족
RACE_DRAGON			=0x2000		--드래곤족
RACE_BEAST			=0x4000		--야수족
RACE_BEASTWARRIOR	=0x8000		--야수전사족
RACE_DINOSAUR		=0x10000	--공룡족
RACE_FISH			=0x20000	--어류족
RACE_SEASERPENT		=0x40000	--해룡족
RACE_REPTILE		=0x80000	--파충류족
RACE_PSYCHO			=0x100000	--사이킥족
RACE_DIVINE			=0x200000	--환신야수족
RACE_CREATORGOD		=0x400000	--창조신족
RACE_WYRM			=0x800000	--환룡족
RACE_CYBERSE		=0x1000000	--사이버스족
RACE_ILLUSION       =0x2000000  --환상마족
RACE_CYBORG			=0x4000000	--사이보그족
RACE_MAGICIAL_KNIGHT=0x8000000	--마도기사족
RACE_HIGH_DRAGON	=0x10000000	--하이드래곤족
--Reason 卡片到当前位置的原因
REASON_DESTROY		=0x1		--효과로 파괴되었다
REASON_RELEASE		=0x2		--릴리스되었다
REASON_TEMPORARY	=0x4		--일시적으로 제외되었다
REASON_MATERIAL		=0x8		--소재 및 재물이 되었다
REASON_SUMMON		=0x10		--소환
REASON_BATTLE		=0x20		--전투
REASON_EFFECT		=0x40		--효과
REASON_COST			=0x80		--코스트로
REASON_ADJUST		=0x100		--패 조정에 의해 버려졌다
REASON_LOST_TARGET	=0x200		--대상을 잃어버렸다
REASON_RULE			=0x400		--룰에 의해 파괴되었다
REASON_SPSUMMON		=0x800		--특수소환
REASON_DISSUMMON	=0x1000		--소환에 실패했다
REASON_FLIP			=0x2000		--리버스했다
REASON_DISCARD		=0x4000		--버려졌다
REASON_RDAMAGE		=0x8000		--데미지를 회복으로 번전하는 효과로 회복했다
REASON_RRECOVER		=0x10000	--회복을 데미지로 반전하는 효과로 데미지를 받았다
REASON_RETURN		=0x20000	--묘지로 되돌렸다
REASON_FUSION		=0x40000	--융합 소환의 소재가 되었다
REASON_SYNCHRO		=0x80000	--싱크로 소환의 소재가 되었다
REASON_RITUAL		=0x100000	--의식 소환의 제물이 되었다
REASON_XYZ			=0x200000	--엑시즈 소환의 소재가 되었다
REASON_REPLACE		=0x1000000	--그 대신에 효과로 그 대신으로 하여 묘지로 되돌리거나 제외되었다
REASON_DRAW			=0x2000000	--드로우했다
REASON_REDIRECT		=0x4000000	--발송지를 변경하는 효과를 받았다
REASON_REVEAL		=0x8000000	--덱에서 넘겼다
REASON_LINK			=0x10000000	--링크 소환의 소재가 되었다
--Location Reason
LOCATION_REASON_TOFIELD		=0x1	--Duel.GetLocationCount()预设值，皇帝斗技场
LOCATION_REASON_CONTROL		=0x2	--Card.IsControlerCanBeChanged()使用
--Summon Type --召唤类型
SUMMON_TYPE_NORMAL		=0x10000000 --통상 소환(EFFECT_SUMMON_PROC,EFFECT_SET_PROC 可用Value修改数值)
SUMMON_TYPE_ADVANCE		=0x11000000 --어드밴스 소환
SUMMON_TYPE_DUAL		=0x12000000	--듀얼 소환（二重）
SUMMON_TYPE_FLIP		=0x20000000	--반전소환
SUMMON_TYPE_SPECIAL		=0x40000000	--특수 소환(EFFECT_SPSUMMON_PROC,EFFECT_SPSUMMON_PROC_G 可用Value修改数值)
SUMMON_TYPE_FUSION		=0x43000000	--융합 소환
SUMMON_TYPE_RITUAL		=0x45000000	--의식 소환
SUMMON_TYPE_SYNCHRO		=0x46000000	--싱크로 소환
SUMMON_TYPE_XYZ			=0x49000000	--엑시즈 소환
SUMMON_TYPE_PENDULUM	=0x4a000000 --펜듈럼 소환
SUMMON_TYPE_LINK		=0x4c000000 --링크 소환
--Status	--卡片当前状态
STATUS_DISABLED				=0x0001		--효과가 무효화되어 있다
STATUS_TO_ENABLE			=0x0002		--将变成有效
STATUS_TO_DISABLE			=0x0004		--将变成无效
STATUS_PROC_COMPLETE		=0x0008		--한 번 정규 방식으로 소환되어 있다
STATUS_SET_TURN				=0x0010		--이 턴에 세트되었다
STATUS_NO_LEVEL				=0x0020		--레벨을 잡지 않았다
STATUS_BATTLE_RESULT		=0x0040		--伤害计算结果预计要破坏的怪兽
STATUS_SPSUMMON_STEP		=0x0080		--효과로 특수 소환 처리 중
STATUS_FORM_CHANGED			=0x0100		--표시 형식 변경
STATUS_SUMMONING			=0x0200		--소환 중
STATUS_EFFECT_ENABLED		=0x0400		--효과 유효
STATUS_SUMMON_TURN			=0x0800		--전체 회합 소환 / 세트
STATUS_DESTROY_CONFIRMED	=0x1000		--파괴 확정
STATUS_LEAVE_CONFIRMED		=0x2000		--체인 처리 또는 마법 / 함정 처리로 묘지로 보내졌다
STATUS_BATTLE_DESTROYED		=0x4000		--전투 파괴 확정
STATUS_COPYING_EFFECT		=0x8000		--효과 복사
STATUS_CHAINING				=0x10000	--체인 처리 중
STATUS_SUMMON_DISABLED		=0x20000	--소환 무효가 되었다
STATUS_ACTIVATE_DISABLED	=0x40000	--발동 무효가 되었다
STATUS_EFFECT_REPLACED		=0x80000	--효과 대체 (레드 슈프리머시)
STATUS_FUTURE_FUSION		=0x100000	--미래융합특수소환(융합소재효과 유발없음)
STATUS_ATTACK_CANCELED		=0x200000	--공격이 취소되었다
STATUS_INITIALIZING			=0x400000	--초기화
STATUS_ACTIVATED			=0x800000	--효과가 발동되었다
STATUS_JUST_POS				=0x1000000	--표시 형식이 변경되었다
STATUS_CONTINUOUS_POS		=0x2000000	--변경 후 다른 표시 형식으로 변경되었다
STATUS_FORBIDDEN			=0x4000000	--사용불가(금지령)
STATUS_ACT_FROM_HAND		=0x8000000	--패에서 발동했다
STATUS_OPPO_BATTLE			=0x10000000	--상대 몬스터와 싸우다
STATUS_FLIP_SUMMON_TURN		=0x20000000	--이 턴에 반전 소환되었다
STATUS_SPSUMMON_TURN		=0x40000000	--이 턴에 특수 소환되었다
--Assume
ASSUME_CODE			=1
ASSUME_TYPE			=2
ASSUME_LEVEL		=3
...
--Counter	--指示物
COUNTER_WITHOUT_PERMIT		=0x1000	--可以放置在非特定对象的指示物
COUNTER_NEED_ENABLE			=0x2000	--在卡片本身放置上述指示物的标记(卡片守卫)
--Phase	--阶段
PHASE_DRAW			=0x01	--드로우 페이즈
PHASE_STANDBY		=0x02	--스탠바이 페이즈
PHASE_MAIN1			=0x04	--메인 페이즈 1
PHASE_BATTLE_START	=0x08	--전투 개시
PHASE_BATTLE_STEP	=0x10	--전투 스텝
PHASE_DAMAGE		=0x20	--데미지 스텝
PHASE_DAMAGE_CAL	=0x40	--데미지 계산시
PHASE_BATTLE		=0x80	--배틀 페이즈
PHASE_MAIN2			=0x100	--메인 페이즈 2
PHASE_END			=0x200	--엔드 페이즈
--Player	--玩家
...
RESET_EVENT			=0x1000				--指定的条件下重置(一般和下面这些事件配合使用)
...
EFFECT_TYPE_ACTIONS			=0x0008		--触发型，以下類型會自动添加此屬性（对峙的G）
EFFECT_TYPE_ACTIVATE		=0x0010		--魔陷发动
EFFECT_TYPE_FLIP			=0x0020		--反转效果
...
EFFECT_FLAG_CANNOT_NEGATE	=0x0200		--含有"此效果不會被无效"的敘述
...
EFFECT_FLAG_CANNOT_INACTIVATE	=0x2000000	--发动不會被无效
...
EFFECT_CANNOT_FLIP_SUMMON		=21		--不能反转召唤怪兽
...
EFFECT_CANNOT_DISABLE_FLIP_SUMMON	=39	--反转召唤不会无效
...
EFFECT_FLIPSUMMON_COST			=93		--反转召唤代价
...
EFFECT_LPCOST_REPLACE			=171	--以其他动作代替生命值代价
...
EFFECT_FUSION_MATERIAL			=230	--指定融合素材的条件
...
EVENT_DESTROY					=1010	--确定被破坏的卡片移动前
...
EVENT_LEAVE_FIELD_P				=1019	--离场的卡片移动前
EVENT_CHAIN_SOLVING				=1020	--连锁处理开始时（EVENT_CHAIN_ACTIVATING之后）
...
EVENT_CHAIN_NEGATED				=1024	--连锁发动无效时（EVENT_CHAIN_ACTIVATING之后）
...
EVENT_FLIP_SUMMON_SUCCESS		=1101	--反转召唤成功时
...
CATEGORY_LEAVE_GRAVE		=0x4000000	--涉及墓地的效果(王家长眠之谷)
...
TIMING_FLIPSUMMON			=0x100      	--反转召唤时点
...
--Global flag	--特殊标记
GLOBALFLAG_DECK_REVERSE_CHECK	=0x1		--卡组反转标记
